				...
				ulong flags)
{
	...
	for_each_free_mem_range(i, nid, flags, &this_start, &this_end, NULL) {
                // TOT0Ro > start와 end를 벗어나지 않음 && free 영역(this)의
                // start와 end 설정.
		this_start = clamp(this_start, start, end);
		this_end = clamp(this_end, start, end);

                // TOT0Ro > free 영역에 align 적용시킨 공간의 size가
					...
					phys_addr_t end, int nid, ulong flags)
{
	...
	if (memblock_bottom_up() && end > kernel_end) {
		...
		/* ok, try bottom-up allocation first */
                // TOT0Ro > 아래에서 위로 free 영역(memory에 속하지만 reserved에 속하지 않은 영역)
        ...
        // TOT0Ro > 위에서 아래로 한다는 것 만 다르고 __memblock_find_range_bottom_up()과 완전히 동일.
 ...
 */
// TOT0Ro > free 영역을 찾아서 out_start와 out_end에 그 영역을 설정.
	...
	for (; idx_a < type_a->cnt; idx_a++) {
		...
		if (!(flags & MEMBLOCK_NOMAP) && memblock_is_nomap(m))
			continue;
                ...
                // TOT0Ro > next memory 영역으로 왔는데 남은 reserved 영역이 없다?
		...
		for (; idx_b < type_b->cnt + 1; idx_b++) {
			...
			r = &type_b->regions[idx_b];
                        // TOT0Ro > reserved 영역의 start와 end가 아닌
                        // @@@ free 영역의 start와 end를 설정 하는 것. @@@
                        // TOT0Ro > idx_b가 0이면 0을 start로 설정.
                        // 그 외에는 이전 region의 end를 start로 설정.
			r_start = idx_b ? r[-1].base + r[-1].size : 0;
                        // TOT0Ro > idx_b가 region 갯수보다 크면 최댓값을 설정.
			 ...
			 */
                        // TOT0Ro > free 영역이 memory 영역에서 벗어난 상태.
                        // 다음 memory 영역을 참조하도록 for문 탈출.
			if (r_start >= m_end)
				break;
			/* if the two regions intersect, we're done */
                        // TOT0Ro > free 영역이 memory 영역에 겹친 상태.
        ...
        // TOT0Ro > 현재 idx에 해당하는 memblock.type이 memory인 영역이 더이상 없음.
